    ...
    try {
      ...
      const overviewData: (string | number | undefined)[][] = [
        ...
        [
          ...
          cinemaId === "all"
            ? "Tất cả"
            : cinemas.find((c) => c.id === cinemaId)?.name || cinemaId,
        ],
        [""],
      ];
      ...
      const movieDetailData: Record<string, string | number>[] = [];
      moviesList.forEach((movie) => {
        const breakdown = movie.dailyBreakdown || [];
        breakdown.forEach((day: DailyRevenueBreakdown) => {
      ...
      const cinemaDetailData: Record<string, string | number>[] = [];
