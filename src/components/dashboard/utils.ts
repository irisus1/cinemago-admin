...
export const fmtPercent = (n: number | undefined | null) =>
...
export const aggregateByMonth = (
  data: (DailyRevenueBreakdown | DailyRevenueGlobal)[],
): DailyRevenueGlobal[] => {
  if (!data || data.length === 0) return [];

  const groups: Record<
    string,
    DailyRevenueGlobal & { occupancyRateSum: number; count: number }
  > = {};
  ...
  return Object.values(groups).map((g) => {
    ...
    return {
      ...
      occupancyRate:
        count > 0 ? Number((occupancyRateSum / count).toFixed(2)) : 0,
    };
  });
};
